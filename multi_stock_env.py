...
Stocks_Per_Sector={
    'Tech':['INTU'], # |,'HLIT','INTU','ADBE',,'POWI',,'SNX','CPSI','SEAC','SYNA','MSFT','BRKS','MCHP'
    'Energy':['PLUG'], # |,'NGS','COG','PTEN',,'PLUG','CRZO','CPST','CNX','DO','OIS','DRQ','ISRL'
    'Finance':['GS'], # |,'PFG','BANF','CACC','GS','EEFT','CPSS','STFC','CASH','FISI',,'CME','SAFT','COLB'
    'Healthcare':['PDLI'], # |,'MYGN','AMGN','PDLI','ICUI','XRAY','GERN',,'AGEN','EXAS','CNMD','GILD','SGEN'
    'Utilities':['NTGR'], # |'ADTN','CWST','NTGR','USM',,'WSTL','PLT','RSG','CIEN'
    'Transportation':['USAK'], #|'ODFL','HTLD',,'USAK','CHRW','PTSI','WERN','JBLU','HUBG','UPS','LSTR','SKYW',
}
...
class MultiStock(gym.Env):
    def __init__(self, num_sectors):
        ...
        self.start_date = date(2003, 1, 1)
        self.end_date = date(2004, 1, 1)
        self.stock_ind=0
    ...
    def step(self, action_m, action_w):
        ...
        for i, a in enumerate(action_w):
            if a == 0 and self.portfolio[i] - 1 >= 0:  # sell
                self.balance += (prices[i] - TRANS_FEE)
                self.portfolio[i] -= 1
                self.last_price[i] = prices[i]
            elif a == 1 and self.balance - (prices[i] + TRANS_FEE) >= 0:  # buy
                self.balance -= (prices[i] + TRANS_FEE)
                self.portfolio[i] += 1
                self.last_price[i] = prices[i]
            ...
            if (self.balance - (prices[i] + TRANS_FEE) < 0 and a == 1) or (self.portfolio[i] - 1 < 0 and a == 0):
                r = -1
            else:
                # store old val
